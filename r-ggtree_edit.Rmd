...
```{r load_ggtree, eval=FALSE, echo=FALSE}
library(tidyverse)
library(ggtree)
```
...
```{r ex2, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r ex6, echo=FALSE, warning=FALSE, results="hide", message=FALSE, fig.width=8, fig.height=6.5}
p2 <- p + geom_tiplab()
collapse(p2, 18, 'min', color= "orange", fill='steelblue', alpha=.4) %>% 
collapse(23, 'max', fill='firebrick', color='yellow')
